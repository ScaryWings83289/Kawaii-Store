    ...
    MBox.prototype = {
      ...
      build: function () {
        ...
        if (this.imagesCount > 1) {
          ...
          var navigationArrows = $("<div/>")
            .addClass("mBox-navigation")
            .css("display", "none");

          $("<i/>", {
            class:
              "fas fa-chevron-circle-left point mBox-previous float-left hide",
          }).appendTo(navigationArrows);

          $("<i/>", {
            class:
              "fas fa-chevron-circle-right point mBox-next float-right hide",
          }).appendTo(navigationArrows);
          ...
          if (this.settings.displayThumbnails) {
            ...
            this.images.each(function () {
              ...
              $(this).wrap(
                '<div class = "mBox-image" style = "width: ' +
                  width +
                  ';"></div>'
              ); // @ size of the tile should be a size of images per page.
            });
          }
        }
      ...
      createNavigation: function () {
        ...
        if (this.settings.displayTitle == true) {
          var title = this.currentImage.attr("title")
            ? this.currentImage.attr("title")
            : "";
        ...
        if (this.imagesCount > 1) {
          ...
          if (this.imagesCount > this.settings.imagesPerPage) {
            var totalPages = Math.ceil(
              this.imagesCount / this.settings.imagesPerPage
            );
            ...
            if (
              Math.floor(this.currentImageNo / this.settings.imagesPerPage) !==
              this.currentPageNo
            ) {
              var left = Math.floor(
                this.currentImageNo / this.settings.imagesPerPage
              ); // Calculate the right page number.

              $(".mBox-thumbnail-pagination i").removeClass("active");
              $(
                ".mBox-thumbnail-pagination i[data-page-number = " + left + "]"
              ).addClass("active");
      ...
      updateCurrentImage: function (newCurrent) {
        ...
        if (newCurrent.attr("title"))
          this.currentImage.attr("title", newCurrent.attr("title"));
        else this.currentImage.removeAttr("title");

        if (newCurrent.attr("alt"))
          this.currentImage.attr("title", newCurrent.attr("alt"));
      ...
      updateVars: function () {
        ...
        this.currentImageNo = this.getCurrentNumber(
          this.currentImage.attr("src")
        );
      },
      ...
      addEventListeners: function () {
        ...
        $(".mBox-thumbnail-pagination i").on(
          "click",
          $.proxy(function (e) {
            var $this = $(e.target);
      ...
      open: function () {
        if ($(document).find(".body-overlay").length == 0)
          $("body").append('<div class = "body-overlay"></div>'); // Overlay.
